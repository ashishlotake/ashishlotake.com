...
<div align="center">![](/static/images/blog/what-is-data-science/04-tabular-data.png)</div>I hope you
were not like me!
...
We need to make sure all the columns in the datasets must have the same scale, we use **Standardization**.
...
- when we input these values into our algorithm the model will not perform well, so to achieve good performance, we need to make sure all the values must be in the same range.
  **Standardization** will ensure all the numerical columns are in the range of 0 and 1

Normalization will ensure all the columns will have zero mean and unit variance.
